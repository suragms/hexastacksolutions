...
type GuideCard = { title: string; excerpt: string; category: string; slug: string };

const STATIC_GUIDES: GuideCard[] = [
    ...
    { title: 'VAT-Compliant Billing Software for UAE Businesses (2026)', excerpt: 'What UAE and Gulf businesses should check before buying billing software.', category: 'Gulf VAT', slug: '/blog/vat-compliant-billing-software-uae' },
...
function staticPathKey(slugPath: string) {
    return slugPath.replace(/^\/blog\//, '');
}
...
export default function Blog() {
    const [apiPosts, setApiPosts] = useState<{ title: string; excerpt: string; category: string; slug: string }[]>([]);

    useEffect(() => {
        fetch(`${API_URL}/api/blog`)
            .then((r) => (r.ok ? r.json() : []))
            .then((data) => setApiPosts(Array.isArray(data) ? data : []))
            .catch(() => setApiPosts([]));
    }, []);

    const guides = useMemo(() => {
        const fromApi: GuideCard[] = apiPosts.map((p) => ({
            title: p.title,
            excerpt: p.excerpt,
            category: p.category,
            slug: `/blog/${p.slug}`,
        }));
        const apiKeys = new Set(apiPosts.map((p) => p.slug));
        const staticOnly = STATIC_GUIDES.filter((g) => !apiKeys.has(staticPathKey(g.slug)));
